...
class ResNet(Backbone):
    ...
    def featuremaps(self, x):
        ...
        x_aug = self.mixstyle(x_1)
        x_aug = self.layer2(x_aug)
        x_aug = self.mixstyle(x_aug)
        x_aug = self.layer3(x_aug)
        x_aug = self.mixstyle(x_aug)
        

        return self.layer4(x), self.layer4(x_aug)

    def forward(self, x):
        f, f_aug = self.featuremaps(x)
        v = self.global_avgpool(f)
        v_aug = self.global_avgpool(f_aug)

        return v.view(v.size(0), -1), v_aug.view(v_aug.size(0), -1)
