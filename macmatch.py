...
'''
we just simply modified StyleMatch  https://github.com/KaiyangZhou/ssdg-benchmark
we thank their excellent work
'''
...
#         assert cfg.DATALOADER.TRAIN_X.SAMPLER == "SeqDomainSampler"
    ...
    def colorful_spectrum_mix(self, img1, img2, alpha=1.0, ratio=1.0):
        ...
        '''
        this function is modified from https://github.com/MediaBrain-SJTU/FACT
        we thank their excellent work
        we changed the input from numpy to tensor
        input: img1:tensor(B, C, H, W)  img2:tensor(B, C, H, W)
        return: processed img1:tensor(B, C, H, W), i.e, content of img1 mixed with style of img2
        '''

        img1, img2 = img1.cpu(), img2.cpu()
        img1, img2 = np.array(img1), np.array(img2) #convert to numpy
...
#         lam = np.random.uniform(0.5, alpha, (b, 1, 1, 1)) #sample lam for every image
        lam = np.random.rand(b, 1, 1, 1)

        img1_fft = np.fft.fft2(img1, axes=(2, 3))
        img2_fft = np.fft.fft2(img2, axes=(2, 3))
        ...
        img1_abs = np.fft.fftshift(img1_abs, axes=(2, 3))
        img2_abs = np.fft.fftshift(img2_abs, axes=(2, 3))
        ...
        img1_abs = np.fft.ifftshift(img1_abs, axes=(2, 3))

        img21 = img1_abs * (np.e ** (1j * img1_pha))

        img21 = np.real(np.fft.ifft2(img21, axes=(2, 3)))

        img21 = np.clip(img21, 0, 1)
        img21 = torch.from_numpy(np.asnumpy(img21))
        img21 = img21.float()


        return img21.to(self.device)
    ...
    def forward_backward(self, batch_x, batch_u):
        ...
        with torch.no_grad():
            ...
            for k in range(K):
                ...
                z_xu_k, _ = self.G(xu_k)
                z_xu_k = self.C(z_xu_k, stochastic=False)
        ...
        if self.apply_sty:
            ...
            for k in range(K):
                ...
                # Transfer
                xu_k_sty = self.colorful_spectrum_mix(xu_k, xu_k2)
        ...
        for k in range(K):
            ...
            z_x_k, _ = self.G(x_k)
            z_x_k = self.C(z_x_k, stochastic=True)
        ...
        loss_u_sty = 0
        loss_u_fea = 0
#         dropout = nn.Dropout(0.5) # we find that dropout can improve generalization too, you can give it a try
        for k in range(K):
            y_xu_k_pred = y_xu_pred[k]
            mask_xu_k = mask_xu[k]

            # Compute loss for feature augmentation
            x_k = x[k]
            u_k = u[k]
            xu_k = torch.cat([x_k, u_k], 0)
            f_xu_k_fea, _ = self.G(xu_k)
#             f_xu_k_fea = dropout(f_xu_k_fea)
            fea_dis_1 = torch.randn(f_xu_k_fea.shape[0], f_xu_k_fea.shape[1])/2    # this parameter should be changed for different datasets
            fea_dis_2 = torch.randn(f_xu_k_fea.shape[0], f_xu_k_fea.shape[1])/2 + 1
            fea_dis_1, fea_dis_2 = fea_dis_1.to(self.device), fea_dis_2.to(self.device)
            f_xu_k_fea = f_xu_k_fea * fea_dis_2 + fea_dis_1

            
            z_xu_k_fea = self.C(f_xu_k_fea, stochastic=True)
            loss = F.cross_entropy(z_xu_k_fea, y_xu_k_pred, reduction="none")
            ...
            if self.apply_aug:
                ...
                f_xu_k_aug, f_xu_k_aug_mixstyle = self.G(xu_k_aug)
